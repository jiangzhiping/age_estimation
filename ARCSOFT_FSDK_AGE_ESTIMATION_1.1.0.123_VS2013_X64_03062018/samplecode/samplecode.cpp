...
//表示链接libarcsoft_fsdk_age_estimation.lib这个库，和在工程设置里写上链入libarcsoft_fsdk_age_estimation.lib的效果一样，不过这种写法写的程序别人在使用你的代码
//的时候就不用再设置工程settings了。告诉链接器连接的时候要找libarcsoft_fsdk_estimation.lib，这样你就不用在linker的lib设置里指定这个lib了。
